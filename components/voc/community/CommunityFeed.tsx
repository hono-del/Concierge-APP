...
import { getLocalQuestions } from "@/lib/voc/client-store";
import { Badge } from "@/components/voc/ui/badge";
import { Card } from "@/components/voc/ui/card";
import type { ExpertQuestion } from "@/lib/voc/types";
...
interface CommunityFeedProps {
  initialQuestions: ExpertQuestion[];
}

export function CommunityFeed({ initialQuestions }: CommunityFeedProps) {
  const [questions, setQuestions] = useState<ExpertQuestion[]>(initialQuestions);

  useEffect(() => {
    // localStorage に保存されたユーザー投稿をマージ（重複除去）
    const local = getLocalQuestions();
    if (local.length === 0) return;
    const existingIds = new Set(initialQuestions.map((q) => q.id));
    const newOnes = local.filter((q) => !existingIds.has(q.id));
    if (newOnes.length > 0) {
      setQuestions([...newOnes, ...initialQuestions]);
    }
  }, [initialQuestions]);
